...
def db_table_val(user_id: int, user_name: str, user_surname: str, username: str):
	cursor.execute('INSERT OR IGNORE INTO test_table (user_id, user_name, user_surname, username) VALUES (?, ?, ?, ?)', (user_id, user_name, user_surname, username))
	conn.commit()
...
def start_message(message):
    bot.send_message(message.chat.id, f'Привет, {message.from_user.first_name} {message.from_user.last_name} ✌️')
...
def main_buttons(message):
    ...
    bot.send_message(message.chat.id, 'Выбери что ты хочешь получить от Лютика', reply_markup=markup)
...
def vitalik_buttons(message):
    ...
    not_vitalik = types.KeyboardButton('Я не Виталик, я заблудился! Не еби меня :с')
    markup.add(want_meme, want_joke, not_vitalik)
    bot.send_message(message.chat.id, 'Ну шо, Виталя? Ебать тя будем или выберешь что-нибудь другое?', reply_markup=markup)
...
def get_out(message):
    audio_for_user = open('D:/Python/Bot_Cat_Lutik/ba-dum-tss.mp3', 'rb')  # нужно будет записать голос Лютика и отправлять его, если пользователь отправит нам voice
...
def get_user_text(message):
    ...
    if message.text.lower() == 'лютик этого дня':
        ...
        bot.send_message(message.chat.id, 'Дурачок, нажми на любую кнопку снизу. Твои букавы я читать НЕ-БУ-ДУ!')
        main_buttons(message)
